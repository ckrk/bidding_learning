...
class BiddingMarket_energy(gym.Env):
    ...
    def __init__(self, CAP, costs, Fringe=0, Rewards=0, Split=0, past_action = 1, Agents = 3):              
        super(BiddingMarket_energy, self).__init__()
        ...
        # if only Two Agents are learning, doesn't work with Fringe player
        if self.Agents == 2:
        ...
        if self.past_action == 0:
            ...
            if self.Agents == 2:
                self.observation_space = spaces.Box(low=0, high=10000, shape=(3,1), dtype=np.float16)
    ...
    def _next_observation(self, last_action):
        ...
        # including Split Bids are allowed
        if self.Split == 1:
            if self.Agents == 2:
                obs = np.array([Q[0], self.CAP[0], self.CAP[1], last_action[0], last_action[1], last_action[3],
                                last_action[4]])
            else:
        ...
        # without including past action in observation space
        if self.past_action == 0:
            obs = np.array([Q[0], self.CAP[0], self.CAP[1], self.CAP[2]])
            if self.Agents == 2:
                obs = np.array([Q[0], self.CAP[0], self.CAP[1]])
    ...
    def step(self, action, last_action):
        ...
        if self.Agents == 2:
            Sup2 = np.array([int(10), 20000, 20000, 20000, 20000])
        else:
        ...
        # stack all Supplieres together, as required for market_clearing function
        ...
        if self.Split == 1:
            ...
            if self.Agents == 2:
                Sup2 = np.array([int(10), 20000, 20000, 20000, 20000, 20000, 20000])
            else:
        ...
        if self.Fringe == 1 or self.Agents == 2:
