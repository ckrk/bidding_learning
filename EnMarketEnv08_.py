...
class EnMarketEnv08(gym.Env):
    ...
    def _next_observation(self, last_action, last_rewards):
        ...
        obs = np.array([Q[0], self.CAP[0], self.CAP[1], self.CAP[2], last_action[0], last_action[1], last_action[2], 
                        last_rewards[0], last_rewards[1], last_rewards[2]])

        
        return obs

    def step(self, action, last_action, last_rewards):
        ...
        if self.current_step == 0:
            last_action = self.start_action
            last_rewards = self.start_rewards
            obs = self._next_observation(last_action, last_rewards)
        else:
            obs = self._next_observation(last_action, last_rewards)
        ...
        obs = self._next_observation(action, reward)
        last_action = action
        last_rewards = reward
    ...
    def reset(self):
        ...
        self.start_rewards = np.array([0, 0, 0])
        
        return self._next_observation(self.start_action, self.start_rewards)
